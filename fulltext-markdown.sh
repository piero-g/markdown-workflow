...
printf "[$(date +"%Y-%m-%d %H:%M:%S")] fulltext-markdown.sh started running, logging events" >> "$eventslog"
...
######
mkdir -p ./{archive/{original-version,first-conversion,editing-ready},1-layout}
...
tempvar=`mktemp ./tmp-values.XXXXXXXXX.sh`

######
# 2. conversion, change extension, not filename; then archive manuscript
######
printf "\n[$(date +"%Y-%m-%d %H:%M:%S")] Starting conversion of manuscripts from ./0-original..." >> "$eventslog"
( # start subshell
	if cd ./0-original ; then
		...
		if cd ./original ; then
			...
			printf "\n[$(date +"%Y-%m-%d %H:%M:%S")] Moving old ./original to its new name: ./0-original." >> "$eventslog"
			cd ./0-original
		else
			echo "WARNING: ./0-original directory not found!"
			printf "\n[$(date +"%Y-%m-%d %H:%M:%S")] WARNING: ./0-original directory not found! Aborting." >> "$eventslog"
			exit 77
		fi
	fi
	...
	if [ ${#EXT[@]} -gt 0 ]; then
		...
		printf "\n[$(date +"%Y-%m-%d %H:%M:%S")]\t[WARN] No valid files found in ./0-original, exiting now" >> "../$eventslog"
	...
	for manuscript in *{docx,odt} ; do
		if [ "${manuscript}" != "${manuscript%.${EXT1}}" ]; then
			printf "\n[$(date +"%Y-%m-%d %H:%M:%S")]\t${manuscript}: trying to convert it in Markdown..." >> "../$eventslog"
			# actual conversion with Pandoc
			if pandoc --wrap=none --atx-headers -o "../$tempdir/${manuscript%.${EXT1}}.md" "$manuscript" ; then
				printf "\n[$(date +"%Y-%m-%d %H:%M:%S")]\t... ${manuscript} was converted!" >> "../$eventslog"
				# archive the processed manuscript
				mv "$manuscript" "../archive/original-version/${manuscript%.${EXT1}}-$(date +"%Y-%m-%dT%H:%M:%S").${EXT1}"
				printf "\n[$(date +"%Y-%m-%d %H:%M:%S")]\t${manuscript} archived" >> "../$eventslog"
			else
				# pandoc returned errors, print a warning and don't archive
				printf "\n[$(date +"%Y-%m-%d %H:%M:%S")]\t... [WARN] pandoc failed in converting ${manuscript} to Markdown!" >> "../$eventslog"
			...
			if pandoc --wrap=none --atx-headers -o "../$tempdir/${manuscript%.${EXT2}}.md" "$manuscript" ; then
				...
				cp "$manuscript" "../archive/original-version/${manuscript%.${EXT2}}-$(date +"%Y-%m-%dT%H:%M:%S").${EXT2}"
				printf "\n[$(date +"%Y-%m-%d %H:%M:%S")]\t${manuscript} archived" >> "../$eventslog"
			else
				# pandoc returned errors, print a warning and don't archive
				printf "\n[$(date +"%Y-%m-%d %H:%M:%S")]\t... [WARN] pandoc failed in converting ${manuscript} to Markdown!" >> "../$eventslog"
	...
	printf "\n[$(date +"%Y-%m-%d %H:%M:%S")] Original manuscripts are archived in ./archive/original-version with timestamp; something left behind?..." >> "../$eventslog"

	# check if any file is left behind in ./0-original
	for manuscript in * .*; do
		[ -f "$manuscript" ] || continue
		printf "\n[$(date +"%Y-%m-%d %H:%M:%S")]\t[WARN] ${manuscript} was skipped (had errors, wrong extension or it is hidden)" >> "../$eventslog"
		echo WARN=true >> ../$tempvar
	done
) # end subshell
...
( # start subshell
	if cd "./$tempdir" ; then
		...
		printf "\n[$(date +"%Y-%m-%d %H:%M:%S")] WARNING: temporary working directory not found! Aborting." >> "$eventslog"
		exit 77
	fi
	printf "\n[$(date +"%Y-%m-%d %H:%M:%S")] Archiving newly converted manuscripts in ./archive/first-conversion..." >> "../$eventslog"
	for oldname in *.md; do
		# copy to archive
		cp "$oldname" "../archive/first-conversion/${oldname%.md}-$(date +"%Y-%m-%dT%H:%M:%S").md"
		printf "\n[$(date +"%Y-%m-%d %H:%M:%S")]\t$oldname archived" >> "../$eventslog"
	done

	printf "\n[$(date +"%Y-%m-%d %H:%M:%S")] Renaming converted manuscripts..." >> "../$eventslog"
	...
	for oldname in *.md; do
		if [[ $oldname =~ $goodname ]]; then
			...
			printf "\n[$(date +"%Y-%m-%d %H:%M:%S")]\t$oldname renamed as $cleanname" >> "../$eventslog"
		else
			...
			if [[ $oldname =~ $safename ]]; then
				...
				mv "$oldname" "$safename"
				printf "\n[$(date +"%Y-%m-%d %H:%M:%S")]\t[WARN] $oldname has an unexpected name, converted in a safer one!" >> "../$eventslog"
				echo WARN=true >> ../$tempvar
			fi
		fi
	done
	#printf "\n[$(date +"%Y-%m-%d %H:%M:%S")] Conversion finished!" >> "../$eventslog"
	...
	printf "\n[$(date +"%Y-%m-%d %H:%M:%S")] Creating folders for media files in ./1-layout" >> "../$eventslog"
	for f in *.md; do
		fname=$(echo $f | sed -r "s/\.md//")
		if [ ! -d "../1-layout/$fname-media" ]; then
			mkdir "../1-layout/$fname-media"
		else
			RERUN=true
			#printf "\n[$(date +"%Y-%m-%d %H:%M:%S")]\t./archive/$dir/ already there" >> "../$eventslog"
		fi
	done
	if [ $RERUN ]; then
		echo "NOTICE: some files where already parsed before (or something is wrong!)"
		printf "\n[$(date +"%Y-%m-%d %H:%M:%S")]\tsome folders already there, skipped" >> "../$eventslog"
	else
		: # all ok, does nothing
	fi

	# add empty YAML at the start of each article
	printf "\n[$(date +"%Y-%m-%d %H:%M:%S")] Prepending metadata YAML..." >> "../$eventslog"
	yaml_file=../z-lib/article.yaml
	size=$(wc -c < "$yaml_file")

	for file in *.md; do
		if ( cmp -n "$size" "$yaml_file" "$file" &> /dev/null ); then
			printf "\n[$(date +"%Y-%m-%d %H:%M:%S")]\t[WARN] $file has already YAML" >> "../$eventslog"
			echo WARN=true >> ../$tempvar
		else
			tempfile=$(mktemp)
			cat "$yaml_file" "$file" > "$tempfile"
			mv "$tempfile" "$file"
			printf "\n[$(date +"%Y-%m-%d %H:%M:%S")]\t$file has now the empty YAML" >> "../$eventslog"
		fi
	done

	# archive editing-ready manuscripts in ./archive/editing-ready
	printf "\n[$(date +"%Y-%m-%d %H:%M:%S")] Manuscripts are ready, archiving to ./archived/editing-ready/ and moving to ./1-layout/..." >> "../$eventslog"
	for editing in *.md; do
		cp "$editing" "../1-layout/"
		mv "$editing" "../archive/editing-ready/${editing%.md}-$(date +"%Y-%m-%dT%H:%M:%S").md"
		printf "\n[$(date +"%Y-%m-%d %H:%M:%S")]\t$editing is ready" >> "../$eventslog"
	done
	...
	printf "\n[$(date +"%Y-%m-%d %H:%M:%S")] Manuscripts are processed and ready for editing in ./1-layout; each step has been archived with timestamp in ./archive" >> "../$eventslog"
